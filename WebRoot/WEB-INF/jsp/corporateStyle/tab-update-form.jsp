				...
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].corporateName"
											...
											class="textInput required"
											 value="<c:out value="${tmpTmpBean.corporateName}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
				...
				<td>
										...
										<select id="${propName}[${trIndex}].video.id" name="${propName}[${trIndex}].video.id" class=" comboxed">
											...
											<c:forEach items="${commonFileList}" var="commonFile" >
												<option value="${commonFile.id}" ${tmpTmpBean.video.id eq commonFile.id ? 'selected' : ''}><c:out value="${commonFile.manageKey}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>
