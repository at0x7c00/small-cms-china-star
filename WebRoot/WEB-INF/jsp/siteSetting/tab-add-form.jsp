		...
		<input type="hidden" name="rowGuard${trIndex}" value="1"/>
				...
				<td>
										...
										<select id="${propName}[${trIndex}].headerPic.id" name="${propName}[${trIndex}].headerPic.id" class=" comboxed">
											...
											<c:forEach items="${commonFileList}" var="commonFile" >
												<option value="${commonFile.id}" ${tempBean.headerPic.id eq commonFile.id ? 'selected' : ''}><c:out value="${commonFile.manageKey}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>
				<td>
										...
										<select id="${propName}[${trIndex}].infoHeaderPic.id" name="${propName}[${trIndex}].infoHeaderPic.id" class=" comboxed">
											<option value=""><spring:message code="base.common.selectone"/></option>
											<c:forEach items="${commonFileList}" var="commonFile" >
												<option value="${commonFile.id}" ${tempBean.infoHeaderPic.id eq commonFile.id ? 'selected' : ''}><c:out value="${commonFile.manageKey}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>