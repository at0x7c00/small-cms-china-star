								...
								<label class="input">
										<input name="${propName}[${trIndex}].workNum"
											...
											class="textInput required"
											 value="<c:out value="${tempBean.workNum}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
										...
										<select id="${propName}[${trIndex}].photoFile.id" name="${propName}[${trIndex}].photoFile.id" class=" comboxed">
											...
											<c:forEach items="${commonFileList}" var="commonFile" >
												<option value="${commonFile.id}" ${tempBean.photoFile.id eq commonFile.id ? 'selected' : ''}><c:out value="${commonFile.manageKey}"/></option>
											</c:forEach>
										</select><i></i>
										</label>
				</td>
				...
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].area"
											...
											class="textInput required"
											 value="<c:out value="${tempBean.area}"/>"
											maxlength="255" />
								</label>
				</td>
				<td>
								<label class="input">
										<input name="${propName}[${trIndex}].job"
											...
											class="textInput required"
											 value="<c:out value="${tempBean.job}"/>"
											maxlength="255" />
								</label>
				</td>